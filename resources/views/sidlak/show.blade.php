...
<head>
    ...
    <style>
        ...
        .sidlak-page .btn-outline-pink:hover {
            background: #fde7f0;
            color: #d63384;
            border-color: #d63384;
        }
...
<body class="sidlak-page" style="min-height: 100vh; background: linear-gradient(135deg, #f8fafc 0%, #fce4ec 100%);">
    ...
    <div class="container pt-4">
        <div class="mb-4">
            <a href="{{ route('sidlak.index') }}" class="btn btn-secondary"><i class="bi bi-arrow-left"></i> Back to
                Journals</a>
        </div>
    </div>
    ...
    <div class="container py-5">
        <div class="row justify-content-center mb-4">
            <div class="col-12 col-md-10 col-lg-8">
                <div
                    class="card border-0 shadow-lg rounded-4 bg-white position-relative overflow-hidden sidlak-card-hover mb-4">
                    <div class="row g-0 align-items-center">
                        <div class="col-md-5 text-center p-4">
                            @if($journal->cover_photo)
                                <img src="{{ asset('storage/' . $journal->cover_photo) }}" alt="Cover"
                                    class="img-fluid rounded-4 shadow"
                                    style="max-height:340px;object-fit:cover;border-bottom:4px solid #e83e8c;">
                            @else
                                <div class="bg-light border rounded-4 d-flex align-items-center justify-content-center mx-auto"
                        ...
                        <div class="col-md-7 p-4">
                            ...
                            <div class="mb-2">
                                <span class="badge px-3 py-2 fs-6 shadow"
                                    style="background:#e83e8c;">{{ $journal->month }} {{ $journal->year }}</span>
                            </div>
                            @if($journal->print_issn)
                                <div class="mb-2 text-muted small">ISSN: <span
                                        class="fw-semibold">{{ $journal->print_issn }}</span></div>
                            @endif
                            @if(Auth::check() && Auth::user()->role !== 'guest')
                                ...
                                <form action="{{ route('bookmarks.toggle') }}" method="POST"
                                    class="sidlak-journal-bookmark-toggle mt-2">
                                    @csrf
                                    <input type="hidden" name="id" value="{{ $journal->id }}">
                                    <input type="hidden" name="type" value="sidlak_journal">
                                    <button type="submit"
                                        class="btn btn-sm {{ $journalBookmarked ? 'btn-pink' : 'btn-outline-pink' }}">
                                        <i
                                            class="bi {{ $journalBookmarked ? 'bi-bookmark-fill' : 'bi-bookmark' }} me-1"></i>
                ...
                <ul class="nav nav-tabs mb-4 border-0" id="sidlakTab" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active" id="articles-tab" data-bs-toggle="tab"
                            ...
                            aria-selected="true" style="color:#e83e8c;font-weight:600;">Articles</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="about-tab" data-bs-toggle="tab" data-bs-target="#about"
                            ...
                            style="color:#e83e8c;font-weight:600;">About</button>
                </ul>
                <div class="tab-content" id="sidlakTabContent">
                    <div class="tab-pane fade" id="about" role="tabpanel" aria-labelledby="about-tab">
                        <div class="mb-4">
                            ...
                            <div class="mb-2">Published by the Research, Planning, and Development Office, Lourdes
                                College, Cagayan de Oro City</div>
                        </div>
                        ...
                        <div class="mt-3 text-center small">
                            <strong>SIDLAK</strong> (effulgence or radiance) refers to the light that radiates from the
                            sun. It also refers to the quality of being bright and sending out rays of light. SIDLAK
                            publishes relevant studies in various disciplines to advance the development of knowledge
                            and inform diverse professional practice. This volume presents studies in social work,
                            library and information science, hospitality management, home economics, and language
                            teaching.
                        </div>
                        <div class="mt-5 text-center text-muted small">
                            SIDLAK Publication by Lourdes College Research and Planning Development Office is licensed
                            under CC BY 4.0
                            <img src="{{ asset('images/cc.xlarge.png') }}" alt="CC" style="height:1.2em;width:auto;vertical-align:middle;">
                            <img src="{{ asset('images/by.xlarge.png') }}" alt="BY" style="height:1.2em;width:auto;vertical-align:middle;">
                        </div>
                    </div>
                                    ...
                                    <div class="card border-0 shadow sidlak-card-hover h-100">
                                        <div class="card-body">
                                            ...
                                            <div class="d-flex align-items-center flex-wrap gap-2">
                                                <a href="{{ route('sidlak.article.download', $article->id) }}"
                                                    target="_blank" class="btn btn-outline-success">Download PDF</a>
                                                @if(Auth::check() && Auth::user()->role !== 'guest')
                                                    @php
                                                    ...
                                                    @endphp
                                                    <form action="{{ route('bookmarks.toggle') }}" method="POST"
                                                        class="d-inline sidlak-bookmark-toggle"
                                                        data-article-id="{{ $article->id }}">
                                                        @csrf
                                                        <input type="hidden" name="id" value="{{ $article->id }}">
                                                        <input type="hidden" name="type" value="sidlak">
                                                        <button type="submit"
                                                            class="btn btn-sm {{ $isBookmarked ? 'btn-pink' : 'btn-outline-pink' }} sidlak-bookmark-btn">
                                                            <i
                                                                class="bi {{ $isBookmarked ? 'bi-bookmark-fill' : 'bi-bookmark' }} me-1"></i>
                                                            <span
                                                                class="btn-label">{{ $isBookmarked ? 'Bookmarked' : 'Bookmark' }}</span>
                                                        </button>
                                                    </form>
                                                @elseif(!Auth::check())
                                                    <a href="{{ route('login') }}"
                                                        class="btn btn-sm btn-outline-secondary">Login to bookmark</a>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
