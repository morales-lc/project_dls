...
<body style="background: #f7f8fa; min-height: 100vh;">
    ...
    <div class="alert-panel-card shadow rounded-4 p-4 w-100" style="max-width: 800px; background: #fff;">
            ...
            <h2 class="fw-bold mb-4 text-center" style="letter-spacing: 1px; color: #d81b60; font-size: 2rem;">Edit Alert Book</h2>
            <form method="POST" action="{{ route('alert-services.update', $book->id) }}" enctype="multipart/form-data" class="row g-4">
                ...
                <div class="col-md-6">
                    ...
                    <select name="month" class="form-select form-select-lg" required>
                        ...
                        @for($m=1;$m<=12;$m++)
                            <option value="{{ $m }}" @if(old('month', $book->month)==$m) selected @endif>{{ DateTime::createFromFormat('!m', $m)->format('F') }}</option>
                        @endfor
                    </select>
                </div>
                <div class="col-md-6">
                    <label class="form-label">Year</label>
                    <input type="number" name="year" class="form-control form-control-lg" min="2000" max="2100" value="{{ old('year', $book->year) }}" required>
                </div>
                <div class="col-12">
                    ...
                    <select name="department_id" class="form-select form-select-lg" required>
                        ...
                        @foreach($departments as $dept)
                            <option value="{{ $dept->id }}" @if(old('department_id', $book->department_id)==$dept->id) selected @endif>{{ $dept->name }} ({{ ucfirst($dept->type) }})</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-6">
                    ...
                    @if($book->pdf_path)
                        <a href="{{ asset('storage/'.$book->pdf_path) }}" target="_blank" class="d-block mt-1">Current PDF</a>
                    @endif
                </div>
                <div class="col-md-6">
                ...
                <div class="col-12 d-flex justify-content-center mt-2 gap-2">
                    <button class="btn btn-lg px-5 py-2" type="submit" style="font-size:1.1rem; font-weight:600; background:#1976d2; color:#fff; border:none; border-radius:2em;">Update Book</button>
