        ...
        <div class="ms-auto d-flex align-items-center gap-2">
            <div class="dropdown">
                <button class="btn btn-light position-relative d-flex align-items-center justify-content-center rounded-3" type="button" id="adminNotificationDropdown" data-bs-toggle="dropdown" aria-expanded="false" aria-label="New requests" title="New requests" style="width:44px; height:44px; border:1.5px solid #ffd1e3; box-shadow:0 2px 8px rgba(232,62,140,0.06);">
                    ...
                    <span id="adminNotificationTotalBadge" class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger {{ $totalNewRequests > 0 ? '' : 'd-none' }}" style="font-size:0.7rem; min-width:1.2rem;">
                ...
                <ul class="dropdown-menu dropdown-menu-end mt-2" aria-labelledby="adminNotificationDropdown" style="min-width: 250px;">
                    <li class="px-3 py-2 small text-muted d-flex align-items-center justify-content-between">
                        ...
                        <span id="adminNotificationLoadingSpinner" class="d-none" aria-hidden="true">
                    ...
                    <li>
                        <a class="dropdown-item d-flex justify-content-between align-items-center" href="{{ route('alinet.manage', ['status' => 'pending']) }}">
                            ...
                            <span id="adminNotificationAlinetBadge" class="badge bg-danger rounded-pill">{{ $newAlinetCount }}</span>
                        </a>
                    </li>
                    <li>
                        <a class="dropdown-item d-flex justify-content-between align-items-center" href="{{ route('lira.manage') }}">
                            ...
                            <span id="adminNotificationLiraBadge" class="badge bg-danger rounded-pill">{{ $newLiraCount }}</span>
                        </a>
                    </li>
                    <li>
                        <a class="dropdown-item d-flex justify-content-between align-items-center" href="{{ route('feedback.admin') }}">
                            ...
                            <span id="adminNotificationFeedbackBadge" class="badge bg-danger rounded-pill">{{ $newFeedbackCount }}</span>
                        </a>
                    </li>
                    <li>
                        <a class="dropdown-item d-flex justify-content-between align-items-center" href="{{ route('feedback.admin') }}">
                            <span><i class="bi bi-chat-left-text me-2"></i>Comments</span>
                            <span id="adminNotificationFeedbackCommentBadge" class="badge bg-danger rounded-pill">{{ $newFeedbackCommentCount }}</span>
                        </a>
                    </li>
                </ul>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    const dropdownButton = document.getElementById('adminNotificationDropdown');
    const loadingSpinner = document.getElementById('adminNotificationLoadingSpinner');
    const totalBadge = document.getElementById('adminNotificationTotalBadge');
    const alinetBadge = document.getElementById('adminNotificationAlinetBadge');
    const liraBadge = document.getElementById('adminNotificationLiraBadge');
    const feedbackBadge = document.getElementById('adminNotificationFeedbackBadge');
    const feedbackCommentBadge = document.getElementById('adminNotificationFeedbackCommentBadge');
