...
<body>
    ...
    <div class="container py-5">
        <div class="card shadow-lg w-100 border-0" style="max-width:1100px; margin:auto;">
            <div class="card-header bg-pink d-flex align-items-center" style="border-radius:1.25rem 1.25rem 0 0;">
                ...
                <span class="fw-bold fs-5">{{ $category }} Theses</span>
                <div class="ms-auto">
            ...
            <div class="card-body">
                <div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
                    <h4 class="fw-bold mb-2 mb-md-0">Graduate Documents</h4>
                    <div class="view-toggle">
                        <button type="button" class="btn btn-outline-secondary btn-sm active" id="grid-view-btn-graduate"><i class="bi bi-grid-3x3-gap-fill"></i> Grid</button>
                        <button type="button" class="btn btn-outline-secondary btn-sm" id="list-view-btn-graduate"><i class="bi bi-list-ul"></i> List</button>
                    </div>
                </div>

                <form class="row g-2 mb-4 filter-shell" method="GET" action="" id="graduateFilterForm">
                    <div class="col-md-6">
                        ...
                        <div class="tag-discovery-card">
                            <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
                                <div>
                                    ...
                                    <div class="small text-muted">Combine tags to explore graduate theses faster and smarter.</div>
                                </div>
                                <button type="button" class="btn tag-toggle-btn" id="toggleTagSearchGraduate"><i class="bi bi-tags"></i> Hide Tag Search</button>
                            </div>
                            <div id="tagFilterBoxGraduate" class="tag-filter-box">
                                <input type="text" id="tagInputGraduate" class="form-control" list="graduateTagSuggestions" placeholder="Type a tag and press Enter">
                                <datalist id="graduateTagSuggestions">
                                    @foreach(($tagSuggestions ?? []) as $suggestion)
                                        <option value="{{ $suggestion }}"></option>
                                    @endforeach
                                </datalist>
                                <div class="small text-muted mt-2 mb-1">Suggested tags:</div>
                                <div class="suggested-tags" id="suggestedTagsGraduate">
                                ...
                                <input type="hidden" name="tags" id="hiddenTagsGraduate" value="{{ isset($tagFilters) ? implode(',', $tagFilters) : request('tags', '') }}">
                                <div id="selectedTagsGraduate" class="mt-2"></div>
                            </div>
                        </div>
                    </div>
                </form>
                @if($documents->count())
                <div id="graduate-results" class="row g-4 grid-view results-container">
                    ...
                    <div class="col-lg-4 col-md-6 col-sm-12">
                        <div class="card h-100 shadow-sm">
                            <div class="card-body">
                                ...
                                <div class="small mb-2"><span class="text-muted">Category:</span> {{ $doc->category ?? $category }}</div>
                            ...
                            <div class="card-footer bg-white border-0 d-flex justify-content-between align-items-center px-3 py-2">
                                <a href="{{ route('mides.viewer', $doc->id) }}" target="_blank" rel="noopener noreferrer" class="btn btn-outline-secondary btn-sm d-flex align-items-center gap-1" style="border-radius: 8px;">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var gridBtn = document.getElementById('grid-view-btn-graduate');
            var listBtn = document.getElementById('list-view-btn-graduate');
            var container = document.getElementById('graduate-results');
        ...
        (function() {
            var toggleBtn = document.getElementById('toggleTagSearchGraduate');
            var tagBox = document.getElementById('tagFilterBoxGraduate');
            var tagInput = document.getElementById('tagInputGraduate');
            var selectedTagsEl = document.getElementById('selectedTagsGraduate');
            var suggestedTagsEl = document.getElementById('suggestedTagsGraduate');
            var hiddenTags = document.getElementById('hiddenTagsGraduate');
            var tagList = [];
        ...
        (function(){
            async function handleToggle(e){
                ...
                try{
                    ...
                    btn.disabled = false;
                    // ensure spinner removed if still present
